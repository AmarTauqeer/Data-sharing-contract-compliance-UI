export const getServerSideProps = async () => {
  const data = await fetch("http://127.0.0.1:5000/contract/term/types");
...
const index = (props) => {
  ...
  const [typeData, setTypeData] = useState(props.res);
  ...
  const handleChange = (e) => {
    const filtered = typeData.filter((x) => {
  ...
  return (
    <>
      ...
      {typeData !== "No record found for this ID" ? (
        <DataTable
          columns={columns}
          data={filterTypes.length >= 1 ? filterTypes : typeData}
          pagination
